...
function showSidebar()	{
	...
	for (i = 0; i < Lis.length; i++) {
    	Lis[i].i = i;
		Lis[i].onmouseover = function () {   //设置鼠标移入时的样式。
		...
		Lis[i].onmouseout = function () {  	//设置鼠标移开时的样式为空。
			this.className = "";
		}
	}
}
...
function initSearchInput() {
  ...
  //为列表中的每一项绑定鼠标经过事件
  $('.searchList div').mouseenter(function(){
	  $(this).css("background", "#eee").siblings().css("background", "");
  });
  //为列表中的每一项绑定单击事件
  $('.searchList div').click(function(){
	  //文本框为选中项的值
	  $(this).parent().parent().find('.searchInput').val($(this).html());
	  //下拉框隐藏
	  $(this).parent().hide();
  });        
  ...
  var searchList = document.getElementsByClassName('searchList');
  document.body.onclick = function(e){
	  e = e || window.event;
	  var target = e.target || e.srcElement;
	  for(var i = 0; i < dropDowns.length; i++) {
		  if(target != dropDowns[i] && target != searchList[i]){
